...
CSVまたはテキスト抽出可能なPDFを読み込み、抽出行を確認したうえで収益試算ワークブックを生成します。直接還元法は、物件の純収益と還元利回りを用いて収益性を検討する方法です。

Excel上で元資料、入力値、数式、計算結果を照合できる、確認可能な「白箱OSS」として公開しています。
...
- **直接還元法_OER**
  OER（Operating Expense Ratio、運営費率）で運営費用を概算するシートです。空室損失率、OER、資本的支出、還元利回りなどを入力し、収益試算値を算定します。
- **直接還元法‗費用詳細版**
  運営費用を項目別に確認・入力して積み上げるシートです。OER版とは入力値・計算値を参照し合わず、独立して計算します。
- **読み取りレントロール**
  抽出した区画、面積、賃料、共益費、付帯収入、月計、年計を確認するシートです。両計算シートが年額収入を参照します。
...
GitHubリポジトリをrepo-hosted Marketplaceとして登録し、Codexの`/plugins`からインストールできます。詳細はCodex Plugin導入ガイドに掲載しています。

**C．Claude Code利用者**

GitHubリポジトリをClaude Code Marketplaceとして登録し、「revenue-kun」Pluginをインストールできます。詳細はClaude Code Plugin導入ガイドに掲載しています。

### 6．local-firstとプライバシー

revenue-kunはホスティング型サービスではなく、利用者のローカル環境で動作します。Local Web UIは`127.0.0.1`のみにbindします。

入力CSV／PDFを外部処理サービスへ送信せず、アプリケーションtelemetryやCookieを含まず、APIキーも要求しません。依存パッケージ取得など利用者が開始する通常の通信は別です。公開repositoryのサンプルは合成データです。
...
- 投資、法律、税務に関する助言ではありません
- 抽出結果、元資料、前提条件、生成Excelを照合し、最終判断は利用者と必要に応じて各分野の専門家が行います

### 8．開発背景

不動産実務では、計算結果だけでなく、どの資料から何を読み取り、どの前提や数式で算定したかを説明できることが重要です。生成AIを入口として使う場合でも、実務の判断材料をブラックボックスにしない設計が求められます。

revenue-kunは、不動産鑑定・不動産アセットマネジメントの実務と生成AIを接続し、元資料、抽出値、計算式を確認できる基盤を目指しています。合成サンプルと公開コードで検証できるOSSとして開発しています。

### 9．今後の展開

今後も白箱OSSとして改善を継続し、不動産実務者や開発者からのフィードバックを受け付けます。また、不動産実務への生成AI導入やPoC（概念実証）設計に関する相談にも対応します。OCRやスキャンPDFは現時点では対応範囲外です。
...
「不動産の収益試算では、最終的な数字だけでなく、元資料から何を読み取り、どの前提、入力値、数式で計算したのかを確認できることが重要です。revenue-kunは、生成AIの利便性と実務上の説明可能性を両立するため、計算過程をExcelで確かめられる形にしました。OpenAI Plugins Directoryで導入しやすくなったことを機に、実務者や開発者の皆さまからフィードバックをいただき、白箱OSSとして改善を続けます。」
